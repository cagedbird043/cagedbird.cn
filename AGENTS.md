...
- 公众号正文排版使用项目级 `.agents/skills/gzh-design/`：按 `references/theme-index.md` 选择主题并从组件库装配 HTML，不让模型自由编造样式。`md2wechat` 只负责图片素材、封面、摘要、来源链接和草稿等平台操作。
- `source/_posts/` 下的 Markdown 始终是内容源；公众号临时 Markdown、HTML、预览和草稿元数据放在 `/tmp/`，不得反向改写文章或提交进仓库。
- 排版不得遗漏或增删正文实质内容，也不得擅自追加署名、关注引导或点赞转发 CTA。作者、AI 创作说明、原文链接和留言设置按用户给定值保留。
- 长篇技术或专业观点文章优先考虑「石墨极简」；移动端一律优先单列，避免三列目录和不可换行的多列 Flex。
- 最终 HTML 必须运行 `.agents/skills/gzh-design/scripts/validate_gzh_html.py`，修到 0 ERROR、0 WARNING；修改主题组件库时还必须运行 `component_lint.py` 到 0 ERROR。
- 本地预览只是预检。创建草稿前须确认用户已授权；创建后必须在真实微信公众号编辑器检查开头/代码块、每张正文图片、中段标题衔接和结尾/列表，保存并重载后再验一次。
- 禁止自动执行最终群发；最终发布始终由用户确认并操作。
